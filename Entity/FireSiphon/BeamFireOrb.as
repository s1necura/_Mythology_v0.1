...
void onCollision(CBlob@ this, CBlob@ blob, bool solid)
{
    if (solid)
    {
        if (blob !is null && blob.getTeamNum() != this.getTeamNum())
        {
            ...
            {
			this.server_Hit(blob, blob.getPosition(), Vec2f_zero, damage, Hitters::fire, true);
			}
			Sound::Play("Collides.ogg", this.getPosition(),1);
...
void sparks(Vec2f at, f32 angle, f32 damage, f32 angleVariation = 180, f32 velocityVariation = 0.0f)
{
    ...
    for (int i = 0; i < amount; i++)
    {
        ...
        CParticle@ p = ParticlePixel(at, vel, SColor(255, 219, 87, 67), true, XORRandom(8));
...
{
    int amount = damage * 10 + XORRandom(5);
    ...
    for (int i = 0; i < amount; i++)
    {
        ...
        Vec2f vel = getRandomVelocity(angle, damage * 3.0f + velocityVariation * (randFloat - 0.5f), angleVariation);
        //vel.y = -Maths::Abs(vel.y) + Maths::Abs(vel.x) / 3.0f - 2.0f - randFloat;
        CParticle@ p = ParticlePixel(at, vel, SColor(255, 38, 13, 13), true, XORRandom(20));
        p.gravity = Vec2f(0.0f, 0.0f);
    }
}
...
{
    int amount = damage * 10 + XORRandom(0);
