...
void onTick(CBrain@ this)
{
	...
	if (delay == 0)
	{
		...
		if (target !is null)
		{
            Vec2f direction = target.getPosition() - blob.getPosition();
            f32 currentDistance = direction.Length();
            CMap@ map = getMap();
			...
			if (currentDistance > 200 && !(isVisible(blob, target)))
			{
				this.SetTarget(null);
				return;
			}
			...
			if (blob.get_u32("fire time") + ((XORRandom(3*30))) < getGameTime()) //(7*30)
			{
				blob.set_u32("fire time", getGameTime());
			...
			// chase target
            if (currentDistance < 120 && isVisible(blob, target))
            {
                Vec2f away = blob.getPosition() - target.getPosition();
                away.Normalize();

                Vec2f retreatPos = blob.getPosition() + away * (120 - currentDistance);
                retreatPos.x = Maths::Clamp(retreatPos.x, 0.0f, dim.x);
                retreatPos.y = Maths::Clamp(retreatPos.y, 0.0f, dim.y);
                FlyTo(blob, retreatPos);
            }
            else
            {
