...
void onTick(CBlob@ this)
{
	...
	if (this.getCurrentScript().tickFrequency == 1)
	{
		...
		this.SetLightColor(SColor(255, 255, 255, 155));
...
void onCollision(CBlob@ this, CBlob@ blob, bool solid)
{
    if (solid)
    {
        if (blob !is null && blob.getTeamNum() != this.getTeamNum())
        {
            ...
            {
			this.server_Hit(blob, blob.getPosition(), Vec2f_zero, damage, Hitters::water_stun_force, true);
			}
...
void sparks(Vec2f at, f32 angle, f32 damage, f32 angleVariation = 180, f32 velocityVariation = 0.0f)
{
    ...
    for (int i = 0; i < amount; i++)
    {
        ...
        CParticle@ p = ParticlePixel(at, vel, SColor(255, 255, 216, 0), true, XORRandom(8));
