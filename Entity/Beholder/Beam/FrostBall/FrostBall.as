...
const f32 MIN_FROZEN_TIME = 5.0f;
const f32 MAX_FROZEN_TIME = 10;
...
void onTick( CBlob@ this)
{
	...
	if (this.getTickSinceCreated() < 1)
	{
		...
		this.SetLightColor(SColor(255, 114, 191 , 229));
    ...
    SColor lightColor = SColor( 255, 114, 191, 229);
...
{	
	...
	else
	{
		if(isServer())
		{
			CBlob@ icePrison = server_CreateBlob( "ice_prison", 10, this.getPosition() );
...
{	
	...
	if(isServer())
	{
		CBlob@ icePrison = server_CreateBlob( "ice_prison", 10, blobPos-Vec2f(0,8) );
		if ( icePrison !is null )
		{
	...
	if (map.getBlobsInRadius(thisPos, 24.0f, @blobsInRadius))
	{
		for (uint i = 0; i < blobsInRadius.length; i++)
		{
			...
			if (b !is null)
			{
				...
				if (b.get_bool("waterbarrier") || b.isInWater()) damage *= 0.00f;
