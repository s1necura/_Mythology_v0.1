...
class PNGLoader
{
	...
	void handlePixel(const SColor &in pixel, int offset)
	{
		...
		else
		{
			...
			case map_colors::Beholder:   autotile(offset); spawnBlob(map, "beholder", offset,   3, false); break;
			case map_colors::FireSiphon:   autotile(offset); spawnBlob(map, "firesiphon", offset,   3, false); break;
			case map_colors::Automaton:   autotile(offset); spawnBlob(map, "automaton", offset,   3, false); break;
			case map_colors::Zombie:   autotile(offset); spawnBlob(map, "zombie", offset,   3, false); break;
			case map_colors::Greg:   autotile(offset); spawnBlob(map, "greg", offset,   3, false); break;
			case map_colors::Kraken:   autotile(offset); spawnBlob(map, "kraken", offset,   3, false); break;
			case map_colors::Kobold:   autotile(offset); spawnBlob(map, "kobold", offset,   3, false); break;

			//Cave Decorative Blocks
			case map_colors::Corpse: autotile(offset); spawnBlob(map, "corpse", offset, 255, true); break;


			//Decorative Blocks
			case map_colors::ArcL: autotile(offset); spawnBlob(map, "arcl", offset, 255, true); break;
			case map_colors::ArcR: autotile(offset); spawnBlob(map, "arcr", offset, 255, true); break;

			case map_colors::Bake: autotile(offset); spawnBlob(map, "bake", offset, 255, true); break;
			case map_colors::Flue: autotile(offset); spawnBlob(map, "flue", offset, 255, true); break;
			case map_colors::Bed: autotile(offset); spawnBlob(map, "bed", offset, 255, true); break;
			case map_colors::Table: autotile(offset); spawnBlob(map, "table", offset, 255, true); break;
			case map_colors::Bookshelf: autotile(offset); spawnBlob(map, "bookshelf", offset, 255, true); break;
			case map_colors::ArmorStand: autotile(offset); spawnBlob(map, "armorstand", offset, 255, true); break;
			case map_colors::WeaponStand: autotile(offset); spawnBlob(map, "weaponstand", offset, 255, true); break;
			case map_colors::Window: autotile(offset); spawnBlob(map, "window", offset, 255, true); break;
			case map_colors::Anvil: autotile(offset); spawnBlob(map, "anvil", offset, 255, true); break;
			case map_colors::Firewood: autotile(offset); spawnBlob(map, "firewood", offset, 255, true); break;
			case map_colors::PottedPlant: autotile(offset); spawnBlob(map, "pottedplant", offset, 255, true); break;


			case map_colors::BackBushyTree:
			case map_colors::BackBushyTree + (1 << 4):
			case map_colors::BackBushyTree + (2 << 4):
			case map_colors::BackBushyTree + (3 << 4):
				autotile(offset);
				offsets[back_tree_offset].push_back(offset);
			break;

			case map_colors::Tiledirt: autotile(offset); spawnBlob(map, "tiledirt", offset, 255, true); break;
			case map_colors::HayLeft1: autotile(offset); spawnBlob(map, "hayleft1", offset, 255, true); break;
			case map_colors::HayLeft2: autotile(offset); spawnBlob(map, "hayleft2", offset, 255, true); break;
			case map_colors::HayLeft3: autotile(offset); spawnBlob(map, "hayleft3", offset, 255, true); break;
			case map_colors::HayRight1: autotile(offset); spawnBlob(map, "hayright1", offset, 255, true); break;
			case map_colors::HayRight2: autotile(offset); spawnBlob(map, "hayright2", offset, 255, true); break;
			case map_colors::HayRight3: autotile(offset); spawnBlob(map, "hayright3", offset, 255, true); break;
			case map_colors::WoodenBeam: autotile(offset); spawnBlob(map, "woodenbeam", offset, 255, true); break;
			case map_colors::WoodenBeamR: autotile(offset); spawnBlob(map, "woodenbeamr", offset, 255, true); break;
			case map_colors::SideBeam: autotile(offset); spawnBlob(map, "sidebeam", offset, 255, true); break;
			case map_colors::SideBeamR: autotile(offset); spawnBlob(map, "sidebeamr", offset, 255, true); break;


			case map_colors::HollowKeg: autotile(offset); spawnBlob(map, "hollowkeg",   offset); break;
			case map_colors::SmallCrate: autotile(offset); spawnBlob(map, "smallcrate",   offset); break;
			case map_colors::Sake: autotile(offset); spawnBlob(map, "sake",   offset); break;
			case map_colors::Boiler: autotile(offset); spawnBlob(map, "boiler",   offset); break;
			case map_colors::SeedCrate: autotile(offset); spawnBlob(map, "seedcrate",   offset); break;
			case map_colors::ShelfofIngredients: autotile(offset); spawnBlob(map, "shelfofingredients",   offset); break;

			//Entity
			case map_colors::Trader: autotile(offset); spawnBlob(map, "trader",   offset, 1); break;
	...
	void handleOffset(int type, int offset, int position, int count)
	{
		switch (type)
		{
		case back_tree_offset:
		{
			// load trees only at the ground
			if(!map.isTileSolid(map.getTile(offset + map.tilemapwidth))) return;

			CBlob@ tree = server_CreateBlobNoInit( map_random.NextRanged(35) < 21 ? "back_tree_bushy" : "back_tree_bushy" );
			if(tree !is null)
			{
				tree.Tag("startbig");
...
void getInfoFromBlob(CBlob@ this, SColor &out color, Vec2f &out offset)
{
	...
	else if(name == "back_tree_bushy" || name == "back_tree_bushy")
	{
		color = map_colors::BackBushyTree;
	}
	// FAUNA
	else if(name == "bison")
	{
