...
void main() 
{
    ...
    vec2 fogparra = (gl_FragCoord.xy - vec2(screenWidth / 2.0, screenHeight / 2.0)) * zoomlevel;
    fogparra.x += gametime;
    ...
    strength = strength * 0.05 + noiseval * 0.3;
    ...
    tex.rgb = tex.rgb * (1.0 - strength) + vec3(1.0 * brightness, 1.0 * brightness, 0.8 * brightness) * strength;
